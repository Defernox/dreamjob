...
export default function OffreDetail() {
  ...
  if (isLoading) return <p className="text-sm text-encre-500">Chargement de l&apos;offre…</p>
  if (isError) {
    return (
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_20rem] items-start">
        <div className="space-y-5">
          <div className="bg-surface rounded-carte border border-craie-200 shadow-carte p-5">
            <div className="flex gap-4">
              ...
              <div className="min-w-0">
                {/* Le verdict en toutes lettres : « 91 » ne se lit qu'en
                    connaissant les seuils, « correspond bien » se lit seul. */}
                {offre.score !== null && (
                  <p className={`text-xs font-semibold uppercase tracking-wide mb-1
                                 ${verdictScore(offre.score, reglages?.scoring.seuils.bon,
                                                reglages?.scoring.seuils.moyen).texte}`}>
                    {verdictScore(offre.score, reglages?.scoring.seuils.bon,
                                  reglages?.scoring.seuils.moyen).libelle}
                  </p>
                )}
                <h1 className="text-[22px] font-bold tracking-tight leading-snug text-encre-950">
                  {offre.titre}
                </h1>
                <p className="text-encre-600">{offre.entreprise || '—'}</p>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
                  ...
                  {offre.url && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-sm text-encre-600 underline underline-offset-2 hover:text-encre-900 ml-1"
                    >
                      Source ↗
                    </a>
                  )}
          ...
          <section className="bg-surface rounded-carte border border-craie-200 shadow-carte p-5">
            ...
            {/* max-w-prose : au-dela d'environ 75 caracteres, l'oeil perd la
                ...
                souvent plusieurs milliers de caracteres. */}
            <p className="text-[13.5px] text-encre-700 whitespace-pre-wrap leading-[1.75] max-w-prose">
        ...
        <aside className="bg-surface rounded-carte border border-craie-200 shadow-carte p-5 space-y-4 lg:sticky lg:top-5">
          ...
          <div className="space-y-2">
            ...
            {documents.isSuccess && (
              <div className="rounded-md bg-succes-50 border border-succes-200 px-3 py-2.5 text-xs text-succes-900">
                ...
                {documents.data.mots_cles_non_couverts.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-succes-200">
                    <p className="font-medium text-succes-900">
          ...
          <div className="space-y-2 pt-2 border-t border-craie-200">
            {dejaPostule ? (
              <div className="rounded-md bg-succes-50 border border-succes-200 px-3 py-2.5 text-sm text-succes-900">
                <p className="font-medium">✓ Candidature enregistrée</p>
                <p className="mt-1 text-succes-800">
          ...
          <dl className="text-xs text-encre-500 space-y-1 pt-2 border-t border-craie-200">
...
const Retour = () => (
  <Link to="/offres" className="text-sm text-encre-500 hover:text-encre-900">
    ← Retour aux offres
  </Link>
)

const Etiquette = ({ children }: { children: React.ReactNode }) => (
  <span className="px-1.5 py-0.5 rounded text-xs bg-craie-200 text-encre-600">{children}</span>
)

const Ligne = ({ libelle, valeur }: { libelle: string; valeur: string }) => (
  <div className="flex justify-between gap-3">
    <dt>{libelle}</dt>
    <dd className="text-encre-600">{valeur}</dd>
  </div>
)
